...
def get_instructors_by_discipline_chart(workout_df: pd.DataFrame) -> px.bar:
    ...
    workout_df = (
        workout_df.groupby(["Instructor Name", "Fitness Discipline"])["count"]
        .agg("count")
        .reset_index()
    )
    chart = px.bar(
...
def get_cycling_class_length_chart(
    start_date: datetime, end_time: datetime
) -> px.scatter:
    workout_df = df.loc[
        (df["Workout Timestamp"] >= start_date) & (df["Workout Timestamp"] <= end_time)
    ]
    length_by_calories_df = workout_df.dropna(
        axis="rows", how="any", subset=["Length (minutes)"]
    ).reset_index(drop=True)
    length_by_calories_df = length_by_calories_df.loc[
        length_by_calories_df["Fitness Discipline"] == "Cycling"
    ]
    bx = px.box(
...
def get_top_instructor(workout_df: pd.DataFrame) -> px.bar:
    ...
    fav_instructor_image = os.path.join(
        assets_dir, img_dir, fav_instructor_name.lower().replace(" ", "_") + ".png"
    )
    return html.Center(
        html.Img(
    ...
    return dbc.Card(
        [
            dbc.CardBody(
                [
                    html.Center(
                        html.H1("Calories by Class Length", className="card-title")
                    ),
                    html.Center(
                        dcc.DatePickerRange(
                    ...
                    dcc.Graph(id="class-length-by-calories-chart"),
                ]
            )
        ],
        color="info",
...
def create_discipline_card(workout_df: pd.DataFrame) -> dbc.Card:
    return dbc.Card(
        ...
        color="info",
        outline=True,
        style={"width": "40rem", "margin-left": "1rem", "margin-bottom": "1rem"},
    )
        ...
        [
            dbc.CardBody(
                [
                    ...
                    html.Center(
                        html.H3(f"{fav_instructor_name} is the top instructor!")
                    ),
        ...
        [
            dbc.CardBody(
                [
                    ...
                    dcc.Graph(id="weekly-calories-burned-chart"),
                ]
            )
        ],
        outline=True,
    ...
    children=[
        ...
        dbc.Row(
            [
                ...
                dbc.Col([create_calories_card(df), create_discipline_card(df)]),
            ]
        ),
...
def update_weekly_calories_burned_chart(start_date, end_date):
    dff = df.loc[
        (df["Workout Timestamp"] >= start_date) & (df["Workout Timestamp"] <= end_date)
    ]
    for tz in ["EST", "EDT", "-04", "-05"]:
        dff["Workout Timestamp"] = dff["Workout Timestamp"].str.replace(f"\({tz}\)", "")
    dff["Workout Timestamp"] = pd.to_datetime(
        dff["Workout Timestamp"], format="%Y-%m-%d %H:%M", errors="coerce"
    )
    calories_df = (
        dff.groupby(pd.Grouper(key="Workout Timestamp", freq="W"))[
    ...
    line.add_trace(
        go.Scatter(
            x=calories_df["Workout Timestamp"],
            y=calories_df["Calories Burned"],
